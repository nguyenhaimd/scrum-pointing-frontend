...
export default function ScrumPointingApp() {
  ...
  const join = () => {
    if (nickname.trim() && room.trim()) {
      ...
      socket.emit('join', { nickname, room, role, avatar });
      setHasJoined(true);
    }
  };
      ...
      <div className="max-w-2xl mx-auto bg-white shadow-xl rounded-2xl p-6">
        {!hasJoined ? (
          <div className="text-center">
            ...
            <select className="p-2 border rounded w-full mb-4" value={role} onChange={(e) => setRole(e.target.value)}>
              {ROLE_OPTIONS.map((r) => (
                <option key={r} value={r}>{r}</option>
              ))}
            </select>
            <button className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition" onClick={join}>Join</button>
          </div>
        ) : (
          <>
